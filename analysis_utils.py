...
def process_df(train_df, val_df, test_df, params=None):
    loss_metrics = []
    acc_metrics = []
    for group_idx in range(4):   ## params['n_groups'] -> n_groups = 6
...
def load_log(run_dir):
    ...
    for split in ['train', 'val', 'test']:
        log_path = os.path.join(run_dir, f'{split}.csv')
...
def get_accs_for_epoch_across_batches(df, epoch):
    ...
    print("robust_acc: ", robust_acc)
    print("avg_acc: ", avg_acc)
    return avg_acc, robust_acc


def get_accs_for_epochs(df, epochs):
    n_groups = 1 + np.max([int(col.split(':')[1]) for col in df.columns if col.startswith('avg_acc_group')])
    print("n_groups: ", n_groups)
...
if __name__ == "__main__":
    run_dir = "/root/meta_mixup_DRO/logs/mnli_gdro_uniform_epochs20_lr1e-5_weight0.5"
    dfs = load_log(run_dir)
    dfs_dict = {"train": dfs[0], "val": dfs[1], "test": dfs[2]}
    process_df(dfs_dict["train"], dfs_dict["val"], dfs_dict["test"])
    # print_accs(dfs_dict)
    # get_accs_for_epochs(dfs_dict["train"], 12)
    # get_accs_for_epochs(dfs_dict["val"], 14)
    get_accs_for_epochs(dfs_dict["test"], 4)
